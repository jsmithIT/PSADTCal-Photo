...
Try {
	...
	[string]$appVendor = 'California Department of Justice'
	[string]$appName = 'Cal-Photo'
	...
	$64bitAppInstallName = "Cal-Photo"
	# Application install Path.
	$64bitAppInstallPath = "${Env:ProgramFiles}\Tuolumne County\Cal-Photo"

	##* Application uninstall parameters.
	# 64-bit application
	# 64-bit application uninstall name.
	$64bitAppUninstallName = "Cal-Photo"
	...
	[string[]]$appShortcutDirs = @("${Env:ProgramFiles}\Tuolumne County\Cal-Photo", "${Env:ProgramData}\Microsoft\Windows\Start Menu\Programs", "${Env:PUBLIC}\Desktop")
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Foreach ($appShortcutDir in $appShortcutDirs) {
			Try {
				...
				New-Shortcut -Path "$appShortcutDir\$64bitAppInstallName.lnk" -TargetPath "CONFIDENTIAL" -IconLocation "$64bitAppInstallPath\Assets\calphoto.ico" -WindowStyle "Maximized" -Description "Cal-Photo"
			}
